...
class TestSemantics {
  ...
  bool _matches(
    ...
    if (childOrder == DebugSemanticsDumpOrder.traversalOrder) {
      childrenCount = node.mergeAllDescendantsIntoThisNode
          ? 0
          : node.childrenCountInTraversalOrder;
    } else {
      childrenCount =
    ...
    if (role != node.role) {
      return fail(
        "expected node id $id to have role $role but found role ${node.role}",
      );
    }
  ...
  String toString([int indentAmount = 0]) {
    ...
    if ((flags is int && flags != 0) ||
        ...
        (flags is SemanticsFlags &&
            (flags as SemanticsFlags) != SemanticsFlags.none)) {
      buf.writeln(
...
class SemanticsTester {
    ...
    bool checkNode(SemanticsNode node) {
      ...
      if (attributedHint != null &&
          (attributedHint.string != node.attributedHint.string ||
              !_stringAttributesEqual(
                attributedHint.attributes,
                node.attributedHint.attributes,
              ))) {
        return false;
      }
      ...
      if (flagsCollection != null) {
        ...
        final SemanticsFlags actualFlags =
  ...
  static String _flagsToSemanticsFlagExpression(flags) {
  ...
  static String _actionsToSemanticsActionExpression(actions) {
  ...
  static String _generateSemanticsTestForNode(
    ...
    if (nodeData.flagsCollection != SemanticsFlags.none) {
      buf.writeln(
        "  flags: ${_flagsToSemanticsFlagExpression(nodeData.flagsCollection)},",
      );
    }
...
class _HasSemantics extends Matcher {
  ...
  @override
  bool matches(
    covariant SemanticsTester item,
    Map<dynamic, dynamic> matchState,
  ) {
    ...
    if (!doesMatch) {
      matchState["would-match"] =
          item.generateTestSemanticsExpressionForCurrentSemanticsTree(
        childOrder,
      );
    }
